...
def VariableRename(tree_root_node,rename_list):
    ...
    variable_no_keyword_list=VaraibleDelKeywords(variable_list)
    variable_name_list=[]
...
def FindVariable(identifier_list):
    ...
    for i in range(0,len(identifier_list)):
        if identifier_list[i].parent.type != 'function_definition' and identifier_list[i].parent.type != 'class_definition' and identifier_list[i].parent.type != 'call' and not(identifier_list[i].parent.type == 'attribute' and identifier_list[i].parent.parent.type == 'call'):
...
def VaraibleDelKeywords(variable_list):
    with open('python/rename/keywords.json') as f:
